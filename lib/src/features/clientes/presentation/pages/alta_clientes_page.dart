...
class _AltaClientePageState extends State<AltaClientePage> {
  ...
  Future<void> _crearCliente() async {
    ...
    try {
      ...
      try {
        ...
        // Si falla la notificaci?f�n no bloqueamos el flujo
      }
      ...
      _mostrarMensaje(
        AppErrorMapper.toUserMessage(
          error,
          fallbackMessage: 'Ocurri?f� un error inesperado al crear el cliente.',
        ),
        esError: true,
      );
    } finally {
  ...
  String? _validarNombreOApellido(String? value, String etiqueta) {
    ...
    final soloLetras = RegExp(
      r'^[A-Za-z?f�?f????f�?f??o?fs?f�?f�?f�?f�?f�?f??~?f�?f?"?f� ]+$',
    );
  ...
  String? _validarDni(String? value) {
    ...
    if (!RegExp(r'^\d{7,8}$').hasMatch(text)) {
      return 'El DNI debe tener 7 u 8 d?f�gitos num?f�ricos.';
    }
    return null;
  }
  ...
  String? _validarPassword(String? value) {
    ...
    if (text.length < 8 || text.length > 32) {
      return 'La contrase?f�a debe tener entre 8 y 32 caracteres.';
    }
    if (!RegExp(r'[A-Z]').hasMatch(text)) {
      return 'Debe contener al menos una letra may?f�scula.';
    }
    if (!RegExp(r'[a-z]').hasMatch(text)) {
      return 'Debe contener al menos una letra min?f�scula.';
    }
    if (!RegExp(r'\d').hasMatch(text)) {
      return 'Debe contener al menos un n?f�mero.';
    }
    if (!RegExp(r'[!@#\$%^&*(),.?":{}|<>_\-]').hasMatch(text)) {
      return 'Debe contener al menos un car?f�cter especial.';
    }
    return null;
  }
      ...
      return Scaffold(
        ...
        body: const Center(
          child: Padding(
            ...
            child: Text(
              'Acceso denegado. Esta funcionalidad est?f� disponible solo para el metre habilitado.',
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        ...
                        counterText: '',
                        // Aqu?f� integramos el bot?f�n de QR que ten?f�as afuera
                        suffixIcon: Padding(
                    ...
                    if (_foto == null) ...[
                      ...
                      const Text(
                        'Foto obligatoria. Se toma con c?f�mara frontal.',
                        textAlign: TextAlign.center,
